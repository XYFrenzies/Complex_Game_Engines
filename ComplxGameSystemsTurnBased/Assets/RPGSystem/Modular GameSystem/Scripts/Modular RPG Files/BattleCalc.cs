...
public class BattleCalc : MonoBehaviour
{
    ...
    public float EffectivenessMath(Moves m_moveType, Entity m_enemyEntity)
    {
        ...
        while (typeItrAtt < m_moveType.m_typeEffectiveness.Count)
        {
            ...
            defense = m_enemyEntity.m_typeEffectiveness[typeItrDef][m_enemyEntity.typeIndex[typeItrDef]];
    ...
    public float EffectivenessLibrary(string m_attack, string m_defense)
    {
        ...
        List<List<string>> typeEAttack = TypesEffected.effected.nameAttack;
        List<List<string>> typeEDefense = TypesEffected.effected.nameDefense;
        for (int i = 0; i < typeEAttack.Count; i++)
        {
            if (typeEAttack[i][TypesEffected.effected.indexValueAtt[i]] == m_attack
                && typeEDefense[i][TypesEffected.effected.indexValueDef[i]] == m_defense)
            {
                if (TypesEffected.effected.effect[i] == Effectiveness.Immune)
                {
                    effectiveness = 0;
                }
                else if (TypesEffected.effected.effect[i] == Effectiveness.None || TypesEffected.effected.effect[i] == Effectiveness.normalEffective)
                {
                    effectiveness = 1;
                }
                else if (TypesEffected.effected.effect[i] == Effectiveness.superEffective)
                {
                    effectiveness = 2;
                }
                else if (TypesEffected.effected.effect[i] == Effectiveness.notVeryEffective)
                {
                    effectiveness = 0.5f;
                }
            }
    ...
    public float DamageIsStab(Entity entity, Moves move)
    {
        ...
        for (int i = 0; i < move.m_typeEffectiveness.Count; i++)
        {
            ...
            {
                if (entity.m_typeEffectiveness[i][entity.typeIndex[i]]
